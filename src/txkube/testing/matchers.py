...
class _MappingLikeMismatch(Mismatch):
    ...
    def describe(self):
        if type(self._actual) != type(self._reference):  # noqa E721
            return (
                "type mismatch:\n"
                "reference = %s (%s)\n"
                "actual    = %s (%s)\n"
            ) % (
                type(self._reference), self._reference, type(self._actual),
                self._actual,
            )
        ...
        return "field mismatch:\n" + "".join(
            ...
            "actual    = %(actual)s\n" % mismatch for mismatch in mismatched
        )
